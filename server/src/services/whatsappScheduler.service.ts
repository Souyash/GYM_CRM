 ...
 * Sends WhatsApp prior notifications for memberships expiring in 5 days, 3 days, 1 day, or expired.
...
export async function runExpiryNotificationCheck(): Promise<{
  ...
  try {
    ...
    // Find subscriptions that are ACTIVE or nearing expiry within 5 days
    ...
    for (const sub of subscriptions) {
      ...
      // 1. Check if 5-Day Prior Notice needs to be sent
      if (daysRemaining > 3 && daysRemaining <= 5 && sub.expiryAlertStage !== 'STAGE_5D') {
        ...
        if (!alreadySentToday) {
          ...
          await prisma.subscription.update({
            where: { id: sub.id },
            data: {
              expiryAlertStage: 'STAGE_5D',
              lastExpiryAlertSentAt: now
            }
          });

          stage5Sent++;
          console.log(`[Expiry Scheduler] 5-Day WhatsApp prior renewal reminder sent to ${user.fullName} (${targetPhone})`);
        }
      }
      // 2. Check if 3-Day Notice needs to be sent
      else if (daysRemaining > 1 && daysRemaining <= 3 && sub.expiryAlertStage !== 'STAGE_3D') {
        const alreadySentToday = sub.lastExpiryAlertSentAt &&
          new Date(sub.lastExpiryAlertSentAt).toDateString() === now.toDateString();
      ...
      // 3. Check if 1-Day Urgent Notice needs to be sent
      ...
      // 4. Check if Expired Notice needs to be sent
...
export function startExpiryScheduler(): void {
  ...
  setTimeout(() => {
    runExpiryNotificationCheck().then((res) => {
      console.log(`[Expiry Scheduler] Initialized: ${res.checkedCount} subscriptions evaluated (${res.stage5Sent} 5-day notices, ${res.stage3Sent} 3-day, ${res.stage1Sent} 1-day, ${res.expiredSent} expired sent).`);
    }).catch(console.error);
  }, 5000);
  ...
  setInterval(() => {
    runExpiryNotificationCheck().then((res) => {
      console.log(`[Expiry Scheduler] Routine run: ${res.checkedCount} subscriptions evaluated (${res.stage5Sent} 5-day notices sent).`);
    }).catch(console.error);
  }, 6 * 60 * 60 * 1000);
}
