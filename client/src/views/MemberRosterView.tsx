  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto font-poppins">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {/* Total Members */}
        <div className="p-4 rounded-2xl bg-white dark:bg-[#0d0d10] border border-slate-200/80 dark:border-zinc-800 shadow-sm">
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <div className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 flex items-center justify-center">
        ...
        {/* Active Passes */}
        <div className="p-4 rounded-2xl bg-white dark:bg-[#0d0d10] border border-emerald-500/30 dark:border-emerald-500/20 shadow-sm">
        ...
        {/* Expired / Due */}
        <div className="p-4 rounded-2xl bg-white dark:bg-[#0d0d10] border border-rose-500/30 dark:border-rose-500/20 shadow-sm">
      ...
      {/* ========================================================================= */}
      <div className="app-card rounded-3xl overflow-hidden shadow-sm border border-slate-200/80 dark:border-zinc-800">
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-zinc-800/80 flex flex-col md:flex-row md:items-center justify-between gap-3 bg-slate-50/50 dark:bg-zinc-900/50">
          
          {/* Segmented Filter Pills */}
          <div className="flex items-center gap-1.5 p-1 bg-slate-200/70 dark:bg-zinc-800 rounded-2xl self-start">
            <button
              type="button"
            ...
            >
              <span>All</span>
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-slate-100 dark:bg-zinc-800">
                {totalCount}
              </span>
            </button>
            ...
            >
              ...
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-emerald-600/20 text-emerald-950 dark:text-emerald-100">
                {activeCount}
              </span>
            </button>
            ...
            >
              <span>Expired</span>
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-rose-600/20 text-rose-900 dark:text-rose-100">
                {expiredCount}
              </span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative flex-1 md:w-72">
              ...
              <input
                ...
                className="bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 w-full focus:outline-none focus:border-emerald-500 shadow-sm"
              />
            </div>
            ...
            <button
              ...
              className="py-2 px-3 rounded-xl bg-white hover:bg-slate-50 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-200 font-bold text-xs flex items-center gap-1.5 border border-slate-200 dark:border-zinc-700 transition disabled:opacity-40 whitespace-nowrap shadow-sm active:scale-95"
        ...
        <div className="md:hidden divide-y divide-slate-100 dark:divide-zinc-800/80">
          {displayedMembers.length === 0 ? (
            ...
            displayedMembers.map((m) => (
              <div key={m.id} className="p-4 space-y-3 hover:bg-slate-50/50 dark:hover:bg-zinc-900/30 transition">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase ${
                      m.isAccessGranted ? 'badge-active-green' : 'badge-alert-coral'
                    }`}
                  >
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-between text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase font-bold">Pass Plan</span>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left text-xs sm:text-sm">
            <thead className="bg-slate-50/80 dark:bg-zinc-900/80 text-slate-500 dark:text-zinc-400 uppercase text-[10px] tracking-wider border-b border-slate-100 dark:border-zinc-800">
              <tr>
                <th className="py-3.5 px-4 font-bold">Athlete</th>
                <th className="py-3.5 px-4 font-bold">Contact Info</th>
                <th className="py-3.5 px-4 font-bold">Membership Plan</th>
                <th className="py-3.5 px-4 font-bold">Membership ID</th>
                <th className="py-3.5 px-4 font-bold">Access Status</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/80">
              {displayedMembers.length === 0 ? (
                ...
                displayedMembers.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-900/50 transition">
                    <td className="py-3.5 px-4 font-semibold text-slate-900 dark:text-white whitespace-nowrap">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-black text-xs flex items-center justify-center">
                          {m.fullName.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <span className="block font-bold">{m.fullName}</span>
                    ...
                    <td className="py-3.5 px-4 text-slate-500 dark:text-zinc-400 text-xs whitespace-nowrap">
                      <div>{m.email}</div>
                      {m.phone && (
                        <div className="text-[10px] text-slate-400 dark:text-zinc-500">{m.phone}</div>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-4 font-mono text-[11px] text-slate-600 dark:text-zinc-300 whitespace-nowrap">
                      IV-{m.id.substring(0, 8).toUpperCase()}
                    </td>
