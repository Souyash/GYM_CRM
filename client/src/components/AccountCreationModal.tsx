...
import { WhatsAppDeliveryPreviewModal } from './WhatsAppDeliveryPreviewModal';
...
export const AccountCreationModal: React.FC<AccountCreationModalProps> = ({
  ...
  const handleSendWhatsAppOtp = async () => {
    const targetPhone = whatsAppPhone.trim() || phone.trim();
    if (!targetPhone) {
      setWhatsAppNotice('Please enter a WhatsApp mobile number.');
      return;
    }
    ...
    try {
      ...
      setWhatsAppNotice(res.message || `A 6-digit WhatsApp OTP was sent to ${targetPhone}.`);
    } catch (err: any) {
      setWhatsAppStep('IDLE');
      setWhatsAppNotice(err.message || 'Failed to dispatch WhatsApp OTP.');
    }
  };
  ...
  const handleVerifyWhatsAppOtp = async () => {
    ...
    if (whatsAppOtpCode.trim().length !== 6) {
      setWhatsAppNotice('Please enter the complete 6-digit WhatsApp OTP.');
      return;
    }
    ...
    try {
      ...
      setWhatsAppStep('IDLE');
      setWhatsAppNotice('✓ WhatsApp verified! The welcome packet & digital invoice will be delivered to this number.');
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-[#0c0c0e] border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl transition-all">
        ...
        <div className="px-6 py-5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-zinc-800 transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6">
          {creationStep === 'SUCCESS' && createdUser ? (
            ...
            <div className="text-center space-y-5 animate-scale-up">
              ...
              {/* WhatsApp Bill Delivery Confirmation */}
              {(createdUser.whatsAppPhone || createdUser.phone || whatsAppPhone) && (
                <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs space-y-2 text-left">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 font-black text-emerald-700 dark:text-emerald-300">
                      <MessageSquare className="w-4 h-4 text-emerald-500 shrink-0" />
                      <span>WhatsApp Bill & Welcome Packet Delivered</span>
                    </div>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-[10px] font-black uppercase text-emerald-700 dark:text-emerald-300">
                      Sent ✓
                    </span>
                  </div>
                  <p className="text-[11px] text-emerald-800/90 dark:text-emerald-200/90 leading-relaxed">
                    Official tax invoice <strong>#{createdUser.subscription?.invoiceNumber || 'INV-2026-ACTIVE'}</strong>, member details, and entrance turnstile guide were dispatched to <strong>{createdUser.whatsAppPhone || createdUser.phone || whatsAppPhone}</strong>.
                  </p>
                  <button
                    type="button"
                    onClick={() => setShowWhatsAppPreview(true)}
                    className="text-[11px] font-bold text-emerald-700 dark:text-emerald-300 hover:underline flex items-center gap-1 cursor-pointer pt-0.5"
                  >
                    Preview WhatsApp Message & Receipt →
                  </button>
                </div>
              )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                {/* WhatsApp Phone & Instant OTP Verification */}
                <div className="p-3.5 rounded-2xl bg-emerald-500/5 dark:bg-emerald-950/20 border border-emerald-500/20 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-slate-700 dark:text-zinc-200 flex items-center gap-1.5 uppercase tracking-wider">
                      <MessageSquare className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                      WhatsApp Tax Invoice & Pass Delivery
                    </label>
                    {isWhatsAppVerified ? (
                      <span className="flex items-center gap-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/30">
                        <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                        Verified ✓
                      </span>
                    ) : (
                      <span className="text-[10px] text-slate-500 dark:text-zinc-400 font-medium">
                        Verify for instant digital bill
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                      />
                    </div>
                    ...
                    {!isWhatsAppVerified && (
                      <button
                        type="button"
                        disabled={!(whatsAppPhone.trim() || phone.trim()) || whatsAppStep === 'SENDING'}
                        onClick={handleSendWhatsAppOtp}
                        className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 transition disabled:opacity-40 shrink-0 shadow-sm"
                      >
                        ...
                        {whatsAppStep === 'SENT' ? 'Resend OTP' : 'Verify WhatsApp'}
                      </button>
                    )}
                  </div>

                  {/* WhatsApp OTP Input Form */}
                  {whatsAppStep === 'SENT' && !isWhatsAppVerified && (
                    <div className="flex items-center gap-2 pt-1 animate-fade-in">
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 transition disabled:opacity-40 shadow-sm"
                      >
                        {isVerifyingWhatsApp ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
                        Confirm
                      </button>
                    </div>
                  )}
      ...
      <WhatsAppDeliveryPreviewModal
        ...
        recipientPhone={createdUser?.whatsAppPhone || createdUser?.phone || whatsAppPhone}
        recipientName={createdUser?.fullName || fullName}
        messageType="WELCOME_BILL"
        invoiceData={{
          invoiceNumber: createdUser?.subscription?.invoiceNumber,
          planName: createdUser?.subscription?.planName || planName,
          price: createdUser?.subscription?.price || price,
          startDate: createdUser?.subscription?.startDate,
          endDate: createdUser?.subscription?.endDate,
          paymentMethod: createdUser?.subscription?.paymentMethod || 'CASH'
        }}
      />
    </div>
  );
